...
- **Explicit turns.** Remote messages never trigger Amp automatically. They appear live in the chat transcript as `[buzz]`-prefixed messages (with author attribution) and become context for your next prompt — but the plugin cancels their turns so they never dispatch inference.
...
**Live incoming messages.** New messages from collaborators appear directly in the Amp chat within seconds (10 s poll) as `[buzz] <author>: …` messages. They never trigger Amp by themselves — the plugin cancels the turn before inference — but they become thread history, so Amp naturally sees them on your next prompt.
